...
2.  In **SQL Editor**, run the following to enable spatial/vector features and create tables:
...
-- 3. Spatial RPC Function (For Deduplication)
create or replace function find_candidates(search_lat float, search_lon float, radius_m float)
returns table (id bigint, heading float, report_count int, embedding text) language sql as $$
  select id, heading, report_count, embedding::text from detections
  where ST_DWithin(geom, ST_SetSRID(ST_MakePoint(search_lon, search_lat), 4326)::geography, radius_m);
$$;
```
